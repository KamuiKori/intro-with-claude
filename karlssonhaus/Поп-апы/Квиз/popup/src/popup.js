...
document.addEventListener('submit',function (e){
    ...
    if(document.querySelector('.form-ready') && document.querySelector('#checkbox').checked && name){
        ...
        if(document.querySelector('#checkbox2').checked){
            carrotquest.identify([{op:'update_or_create',key:'Согласие на получение рекламно-информационных сообщений',value:true}])
        }
        else{
            carrotquest.identify([{op:'update_or_create',key:'Согласие на получение рекламно-информационных сообщений',value:false}])
        }
    }
})
...
document.querySelector('input.next').addEventListener('click', function (e){
    ...
    if(answers != null){
        ...
        carrotquest.identify([{op:'update_or_create',key:cqpopup_name + ' - ' + q,value:answersText.trim()}])
    }
    if(nextSlide){
