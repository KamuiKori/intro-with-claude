var cqpopup_name = "Видео виджет - Авион",
    carrotquest = {
      ...
      resizeFrame: function(obj) {
        var data = { command: "carrotquest.resizeFrame", id: "{{ sending_id }}", width: obj.width, height: obj.height };
        top.postMessage(data, "*");
      }
    };
...
document.addEventListener("readystatechange", function() {
  if (document.readyState === "complete") {
    ...
    carrotquest.resizeFrame({ width: widget.offsetWidth + 'px', height: widget.offsetHeight + 'px' });
  }
});

var sizes = isPhone
  ? { mini: { width: '140px', height: '175px' }, big: { width: '272px', height: '640px' } }
  : { mini: { width: '260px', height: '395px' }, big: { width: '393px', height: '910px' } };
...
document.addEventListener("submit", function(e) {
  ...
  if (isValid) {
    carrotquest.identify({ "$phone": document.querySelector(".cq_popup-input[name='phone']").value });
